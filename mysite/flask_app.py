...
import app.author_scraper as scraper
import db.db_helper as db
import db.models as model
import llm.llmNew as llm
import authorNetworkCode as nw
from app.author_recommender import get_acm_recommendations_and_field_authors
import asyncio
...
def index():
    try:
        ...
        msg = f'The backend is up and running! {rows} researchers in the db.'
        return msg, 200

@app.route('/log/<log_type>')
def server_log(log_type):
    path = (
        f"/var/log/gdpbackend-{environ.get('USER')}.pythonanywhere.com"
        f'.{log_type}.log'
    )
    text = open(path).read()
